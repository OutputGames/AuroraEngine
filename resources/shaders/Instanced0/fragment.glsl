...
void main()
{		
    DEFERRED_POS = vec4(WorldPos,alpha);
    DEFERRED_NORMAL = vec4(normalize(Normal),alpha);

    vec3 diffuse = texture(material.texture_diffuse0, TexCoords).rgb;

    diffuse *= albedo_color;

    DEFERRED_COLOR = vec4(diffuse, alpha);

    DEFERRED_SHADING = vec4(roughness, metallic, 1.0,alpha);
}  